# HTSW

A refined HTS(W) parser, LSP framework, generator, and more.

---

## Table of contents

<!--- TOC -->

- [Developers](#developers)
  - [Language Documentation](#language-documentation)
  - [VSCode Extension](#vscode-extension)
- [Credits](#credits)
- [License](#license)

<!--- END -->

### Developers

#### Language Documentation

Documentation can be found here: [basic-syntax](/docs/getting-started/basic-syntax.md).

#### VSCode Extension

The VSCode Extension can be downloaded from the releases page.

---

## Credits

This project is a continuation of [BusterBrown1218's](https://github.com/BusterBrown1218)
 HTSL language. Housing would not be in the state it is today without his contributions.

## License

HTSW is distributed under the terms of the MIT license. See
[LICENSE.txt](LICENSE.txt) for the full license.

