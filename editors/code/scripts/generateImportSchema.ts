...
const outputPath = resolve(rootDir, "schemas/import.schema.json");
...
if (checkOnly) {
    ...
    if (current !== output) {
        console.error("editors/code/schemas/import.schema.json is stale; run `npm run generate:schema` from editors/code.");
        process.exitCode = 1;
    }
} else {
    writeFileSync(outputPath, output);
    console.log(`generateImportSchema: wrote ${outputPath}`);
}
...
function copyTextOptions(schema: JsonSchema, spec: { description?: string }): void {
