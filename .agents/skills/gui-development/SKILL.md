...
- `right-panel/import-tab/taskController.ts` — `startImport()` (reads per-house trust via `isCurrentHouseTrusted()`), `startExport()`, and `importablesForImport()`. The diff-sink wiring lives here now (was previously in `bottom-toolbar/index.ts`).
