...
- `left-panel/` — three tabs: **Projects** (importables list + Browse/Recent source controls plus the current-house Trust toggle), **Houses** (per-house browser with a house selector, primary Trust control, compact Alias/Detect side actions), and **Settings** (global inventory-button, chat, scrolling, task-sound, completion-sound, and automatic-update preferences).
