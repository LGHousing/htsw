...
- `reparse.ts` — thin DRIVER over `parses.ts`: auto-discovers `import.json`, debounces reloads, polls the authority each tick the overlay is visible, propagates a changed parse into global state. Owns no parsing/snapshot/mtime logic itself.
...
Include groups: an expanded import.json's contents mirror the parse's include structure instead of rendering the merged flat list. `left-panel/importables/includeTree.ts` builds an `IncludeNode` tree (cached per `ParseResult` in a WeakMap) from `parse.gcx.includeEdges` + each importable's declaring file (`importableDeclaringPath` — the importable's own span always sits in the declaring JSON). Included files render as nested collapsible group rows (`includeGroupRow` in rows.ts; expansion lives in `includeGroupExpansion`, where an explicit toggle wins over the default — collapsed normally, expanded while search/type filter narrows; narrowing also hides match-less groups). An import.json that another file in the same source includes loses its own top-level row (`resultsForSource` in tree.ts); mutual includes (a cycle, already a parse error) keep both rows rather than hiding both. Paths inside the parse (`includeEdges`, `IncludeNode.path`) are in the language fileLoader's format — backslashes on Windows — so run them through `canonicalPath` before comparing with GUI-side fullPaths.
