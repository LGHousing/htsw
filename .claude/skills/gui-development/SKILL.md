...
- `render.ts` — single tree renderer + click dispatcher (used by panels and popovers). Two draw-path perf rules live in `drawLaid`: the GL scissor is managed by the item loop (batched across consecutive items sharing a clip rect), NOT inside `renderItem` — and clipped items fully outside their clip on the Y axis are culled before drawing (an unvirtualized list like the chat scrollback otherwise draws its whole backlog every frame). **The cull assumes every element kind draws starting at its rect origin, extending at most max(rect.h, 16)px down** — a new element kind that paints above its rect origin must be exempted or it vanishes near viewport edges. X is never culled (grow text overflows sideways by design). `truncateToWidth` is memoized on (width, text): it runs per truncated label per DRAW frame and binary-searches with font measurements.
...
- `panel.ts` — `Panel` class: bounds, visibility, click trigger, render trigger. Feeds `framePerf.ts` (frame gap + render ms + rebuilt flag per painted frame) for `/htsw debug guiperf`.
...
- `sourceDiff.ts` — STATIC diff producer: per-action `DiffState` comparing source vs the import cache ("what would change vs last import"), for the View tab. Lazy, cached per file — **including "no diff available" results**: the negative memo revalidates against the current house, the parse-cache revision, and `importCache/cache.ts:getImportCacheWriteRevision()` (bumped on content writes/deletes), because a null result can flip without this module hearing about it. Without the negative memo, an unimported file re-ran the whole target/cache lookup for every line of every rebuilt frame. It also positions house-only ghost actions before source lines (or at file end) so static changes render as a unified diff: red `-` old rows and green `+` new rows. `getSourceDiffRevision()` changes whenever any file's stored result changes; `diffDecorator.modelKey` keys the code view's whole-file decoration cache on it.
...
Code-view line-model cache: `codeView.ts` runs `decorateLine` + wrap-row counting over EVERY line to build its `LineModel` (row offsets, id tables, gutter flags), and caches that per scroll id, keyed on (lines array identity, `LineDecorator.modelKey()`, viewport width). Eased-scroll frames mark the whole GUI dirty, so without this cache the O(all lines) pass re-ran per frame — the confirmed big-project scroll-lag cause. **`modelKey()` is a contract: it must change whenever anything `decorateLine` reads changes** (diffDecorator → `getSourceDiffRevision`; progressDecorator → `livePreview.previewRevision` + cursor/phase). Return null to opt out (pass reruns every rebuilt frame). If you add a decorator that reads new state, wire that state's change signal into its key — a stale key renders stale decorations, not a crash. On top of the model cache sits a per-line row-Element cache (`cachedLineRows`, WeakMap keyed by the `RenderableLine`): built rows are reused across frames while their decorations OBJECT (stable under the model cache), layout inputs, and selection slice are unchanged, so a scroll frame only builds Elements for rows that just entered the window. It relies on the model cache handing back identical decoration objects — don't make `buildLineModel` clone decorations.
...
Importables tree perf invariants: the tree is **virtualized** (`renderRows` in `left-panel/importables/tree.ts` materializes only viewport rows from `TreeRow` descriptors) and the descriptor list itself is **cached across frames** (`treeRows()` — rebuilt on a revision bump or a 300ms TTL). Descriptors encode structure only; per-frame state (dots, checkboxes, colors) lives in `content()` closures that still run per visible row per frame. **Any interaction that changes which rows exist — expansion toggles, search, filter, sort, source add/remove — must call `bumpTreeRevision()` (`rowModel.ts`); it also calls `markGuiDirty()` so the retained panel layout rebuilds immediately.** Async changes (reparses, enumeration refreshes) ride the TTL on purpose — but a TTL expiry only actually rebuilds when `treeInputsFingerprint()` (enumerated results + parse object identities) differs from the last build, so steady-state frames don't re-walk the whole tree every 300ms. If a descriptor build ever reads a NEW input that `bumpTreeRevision()` doesn't guard, add it to the fingerprint or its changes will be invisible until something else changes. `/htsw debug treeperf` prints row count + rebuild timings. Relatedly, `memoizedImportableHash` has a 250ms wrapper-keyed front cache so per-row dot checks don't stringify importable metadata every frame; the TTL exists because edit popovers mutate parsed importables in place.
...
The Projects-tab (`left-panel/importables/`) filter popover narrows by importable type and file↔house link status. Its status inputs (house UUID, trust, and cache content + presence revisions) join `treeInputsFingerprint()` only while a status filter is active. The status half of that popover — the link-status list, the checkbox row builder, and the fit-to-widest-label popover width — lives in the shared `left-panel/statusFilter.ts`, so the Importables-tab (`left-panel/houses/`) per-type browser reuses the exact same control: a status-only funnel sits in its search bar (types are already covered by that page's per-type tabs) and narrows the shown rows by each row's `LinkStatusKey`. Don't fork a second status-row renderer; both funnels draw from `statusFilter.ts`.
...
|------|---|---|---|
| `container` | `style: ContainerStyle`, `children: Extractable<Child[]>`, optional `onClick(rect, info)`, `onDoubleClick(rect)`, `onHover(rect, mouseX, mouseY)`, `noClickFlash` | yes if `onClick` or `onDoubleClick` set | `onHover` is observational and does not make a container clickable. `noClickFlash: true` suppresses the white click pulse (`lib/clickFlash.ts`) — for clickable surfaces that are content, not controls (the code-view line rows, where clicks place a selection caret). Otherwise this is the flex-layout primitive used by buttons and rows. |
...
Wheel scrolling is **delta-proportional** and **eased**: the overlay's per-frame wheel poll converts `Mouse.getDWheel()` to notches (`dwheel / 120`, keeping real magnitude — fractional for high-res wheels/touchpads, >1 when a fast flick coalesces into one reading) and every wheel path (panels, popovers, hover cards) receives that float (see **Mouse wheel** below for the poll/suppression split). `dispatchWheel` accumulates `WHEEL_SCROLL_STEP` (3 rows) per notch into the scroll's `target` (via `setScrollTarget`), and `layoutScroll` eases the rendered `offset` toward that target each frame. Don't collapse the delta back to ±1 — that made fast scrolling crawl. Offsets can be fractional; the layout cursor rounds at placement.

The easing exists because wheel input is chunky — a notch is a 3-row (24px) jump, and coalesced flick readings are bigger — so applying the target directly steps instead of glides. `advanceScrollOffset` (in `layout.ts`) does `offset = target + (offset − target)·exp(−dt/τ)` with wall-clock `dt` and `τ = SCROLL_SMOOTH_TAU_MS` (~20ms) — the exponential form is **exact under split dt**, so advancing more than once per frame (the wheel hit-test relays out, then the paint relays out) integrates to the same result; no per-frame guard needed. It snaps (no ease) on first use, after a >100ms gap (tab switch / sub-10-FPS), or within ½px of target. **`layoutScroll` advances the offset BEFORE extracting children** (clamped to last frame's `contentLength`, re-clamped after measuring — the second call is free under split dt): virtualized lists (the Importables tree, the code view) decide which children exist from `state.offset` at extraction time, so extracting first made them materialize for the previous frame's position — a fast flick or the >100ms snap then scrolled the viewport past every materialized row and painted blank until the dirty backstop. Don't move the advance back below the extraction. **Only the wheel is eased.** Thumb-drag (`updateScrollbarDrag`) and every programmatic jump — `setScrollOffset` (chat stick-to-bottom, code-view autofollow, tab-strip edge autoscroll) — set `offset` and `target` together so they track instantly.
...
Two cooperating halves in `overlay.ts`, sharing ONE routing decision — `routeWheel(mx, my, delta, apply)`, precedence: popovers (paint on top; modals absorb wheel anywhere on screen even outside their rect, mirrored by `mouseIsOverPopover` in the hit-test path), then hover cards, then the first panel scroll viewport under the cursor — so the halves can never disagree about which surface owns a wheel movement:

1. **Suppression** — Forge's `GuiScreenEvent$MouseInputEvent$Pre` (registered via `register(ForgeClass, cb)`), firing per `Mouse.next()` event *before* `GuiScreen.handleMouseInput` runs. It reads `Mouse.getEventDWheel()`, hit-tests via `routeWheel(..., apply=false)`, and `cancel(event)`s when one of our surfaces would consume — the only way to stop vanilla `GuiContainer` scroll and `GuiContainerCreative` tab/item-list scrolling. It does NOT apply the wheel.
2. **Application** — `pollWheel()`, a per-frame `Mouse.getDWheel()` poll from the overlay's NORMAL-priority `guiRender` handler. The panel paints at `Priority.LOW`, so the poll's target moves + dirty marks land BEFORE the same frame's paint — moving the poll to LOW-or-later costs one frame of input latency. MC drains the wheel event queue only in `runTick` (~20Hz), so applying from events moved scroll targets in ~50ms steps that the easing only partially masked — 20Hz velocity pulsing, a confirmed "smooth scrolling feels delayed / doesn't update often enough" report. The accumulator refills every FRAME via `Display.processMessages`, so polling it feeds targets at render rate. A poll gap >200ms discards the drained value: the accumulator collects in-world/other-screen wheel that would otherwise land as a phantom scroll on the first overlay frame.

Two easing traps fixed after the poll landed (both in `layout.ts`): `advanceScrollOffset` must treat `dt <= 0` as "advance nothing", NOT snap-to-target — the poll's hit-test relayout and the paint relayout run within the same millisecond, so a dt<=0 snap silently disables easing on exactly the frames that have wheel input (raw per-notch jumps). And `setScrollTarget` refreshes a >100ms-stale `animAt` before moving the target — an idle scroll's `animAt` is up to `GUI_REBUILD_BACKSTOP_MS` old, so without the refresh the first notch after idle hits the gap-snap and lands in one frame instead of easing.

The two channels are independent (`getDWheel()` accumulator vs `getEventDWheel()` per-event; all vanilla handling is per-event), so this neither double-applies nor starves MC. **Keep the halves together:** application without suppression double-scrolls MC surfaces; suppression without application is a dead wheel. A getDWheel-only design (no Pre cancel) was tried long ago and failed for exactly that reason.
...
Live diagnostics are grouped under `/htsw debug <probe>` (see `DEBUG_SUBCOMMANDS` in `slashCommands/index.ts`): `waiters`, `treeperf`, `guiperf [clear]` (rolling overlay frame pacing + rebuild-vs-draw cost from `gui/lib/framePerf.ts` — the probe for "scrolling feels laggy": it splits low fps from expensive rebuilds), `parseperf`, `lagprobe [clear]` (main-thread stall samples ≥250ms from `perf/lagProbe.ts`), `eta`, `trace`. There is no arm-a-sampling-window command anymore (the old `/htsw gui debug <seconds>` was removed; `/htsw gui` now toggles the dashboard). Probe output prints via ChatLib, and client chat lands in MC's `logs/latest.log` as `[CHAT]` lines — so external tooling can read probe results from the log after the user runs the command (verified in production). **Render exceptions are logged ALWAYS to `gui-debug.log` in the deployed module dir**: panel renders, per-popover renders, and tinted `drawImage` calls are individually try/caught and land in the log with a truncated stack via `debugLogError(where, e)`. The drawImage catch also calls `Renderer.finishDraw()` — CT's drawImage can throw from `image.getTexture()` BEFORE its own finishDraw (verified in CT 2.2.1 source), which leaves `Renderer.colorized` set; while set, CT's `drawString`/`drawRect` override every requested color, turning the rest of the frame gray with invisible text. Never call `Renderer.colorize` without an immediately-following draw that completes.
