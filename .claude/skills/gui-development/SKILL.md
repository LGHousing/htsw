...
- **`decorator` is a contextual reserved word in CT 2.2.1's Rhino fork.** Using it as a bare identifier (parameter name or `const decorator = ...`) makes the parser's `primaryExpr()` return null on a nearby identifier; the fork then NPEs at `Parser.memberExpr:3292` (`pn.setLineno(lineno)` on null `pn`, an unconditional call upstream Rhino removed). Symptom: `/ct reload` hangs at 94%, `Error loading module HTSW` + bare `java.lang.NullPointerException` (full stack only on first occurrence; JVM "fast throw" suppresses subsequent ones). The fork added it for the decorators proposal grammar extension. **Property names (`{ decorator: x }`) and member access (`obj.decorator`) are SAFE in source** — only Identifier-position usage triggers it. **BUT** babel-preset-env infers function names from property keys, so `decorator: () => ...` transpiles to `function decorator() {...}` in the bundle — which IS a bare identifier and DOES trigger the bug. So in practice: never write `decorator` as a property key whose value is an inline arrow either. Name props `lineDecorator` / `dec` / etc. See `docs/BUGS.md`.
