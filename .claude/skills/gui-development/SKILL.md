...
- `text-prompt.ts` — `openTextPromptPopover({title, placeholder, prefill, submitLabel, onSubmit})`: the one shared "ask for a string" modal. Use it before writing another bespoke single-input popover (rename-file and the export new-project popover predate it; new-project needs its layout checkbox, so it stays bespoke).
...
- The name-a-new-project prompt lives in `gui/export/newProjectPopover.ts` (opened from the export destination picker, `gui/export/destinationPicker.ts`). It has a "Folder per type" toggle (default ON) that scaffolds `<section>/import.json` includes via `createEmptyProjectFiles(..., {sectionFolders})`; the picker also shows a "New exports → …" preview line and, for flat projects, a "Split into folders…" action (`restructureProjectPerSection`). New-export routing itself is decided in `htsw-editor-common/project` (`sectionFolderImportJson` fallbacks in `exportTargets.ts`), not in the GUI. There is **no** in-game "add importable" popover — new importables are created from the VS Code tools view.
