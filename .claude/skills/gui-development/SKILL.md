...
The Importables filter popover narrows by importable type and file↔house link status. Its status inputs (house UUID, trust, and cache content + presence revisions) join `treeInputsFingerprint()` only while a status filter is active.
