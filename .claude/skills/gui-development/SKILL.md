...
- `reparse.ts` — thin DRIVER over `parses.ts`: auto-discovers `import.json`, loads on active-path change (gentle, snapshot-served), polls the authority each tick the overlay is visible (the poll already picks up ANY disk change via the fingerprint sweep — callers do NOT need to poke the driver after writing files), and propagates a changed parse into global state. `reparseNow()` is the one explicit lever: immediate force-fresh of the active path for user-initiated "I know it changed" moments (recompile, rename). The old `scheduleReparse()` debounce arm is gone — every caller was redundant with the poll, and its force-fresh DELETED the cache entry, leaving a visible no-data hole (tree collapse, vanishing bind chips) until the re-parse landed. Forced refreshes go through `markParseStale` (keeps the last-good parse for readers, re-parses on next authority read); `invalidateParseCacheEntry` is only for files that are actually gone. Owns no parsing/snapshot/mtime logic itself.
...
Wheel scrolling is instant and **delta-proportional** (VS Code-style, no easing): the overlay wheel handler converts `Mouse.getEventDWheel()` to notches (`dwheel / 120`, keeping real magnitude — fractional for high-res wheels/touchpads, >1 when a fast flick coalesces into one event) and every wheel path (panels, popovers, hover cards) receives that float. `dispatchWheel` moves the offset by `WHEEL_SCROLL_STEP` (3 rows) per notch. Don't collapse the delta back to ±1 — that made fast scrolling crawl. Offsets can be fractional; the layout cursor rounds at placement.
