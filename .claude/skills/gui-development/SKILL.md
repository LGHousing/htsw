...
- `left-panel/` — three tabs: **Importables** (importables list + Open file/folder/Browse buttons), **Houses** (per-house browser with a house selector, primary Trust control, compact Alias/Detect side actions), and **Settings** (global Mute import sounds + Auto-proceed imports + Smooth scrolling toggles).
