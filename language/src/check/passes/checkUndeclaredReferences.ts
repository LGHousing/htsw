 ...
 * action points at has to be declared. Otherwise a prune deletes the target and
 * leaves the action pointing at nothing. For functions, menus and regions the
 * import would also create an empty shell for the missing name and prune it
 * again on every run, tripping the watch-mode loop guard.
 */
