...

{
    ...
    const plan = createNestedHydrationPlan([entry], [
        desired(["REQUIRE_ITEM"], ["CHANGE_VAR"]),
    ]);
...

{
    ...
    const plan = createNestedHydrationPlan([unknown, known], [
...
{
    const wrongMatchAny = observed(0, { conditions: ["REQUIRE_ITEM"] }, { matchAny: true });
    const rightMatchAny = observed(1, { conditions: ["REQUIRE_ITEM"] });
    const plan = createNestedHydrationPlan([wrongMatchAny, rightMatchAny], [
        desired(["REQUIRE_ITEM"], []),
    ]);
