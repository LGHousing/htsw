...
export function printDiagnostic(
    sm: htsl.SourceMap,
    diagnostic: htsl.Diagnostic
) {
    const { line, column } = sm.positionAt(diagnostic.span.start);
    const content = sm.lineAt(diagnostic.span.start);
    const len = Math.min(diagnostic.span.end - diagnostic.span.start, content.length - column);
    ...
    ChatLib.chat(`&7${" ".repeat(numberWidth / spaceWidth)}--> source.htsl:${line}:${column}`);
...
function levelToString(level: htsl.DiagnosticLevel): string {
    switch (level) {
        case "bug": return "&c&lbug&r";
        case "error": return "&c&lerror&r";
        case "warning": return "&e&lwarning&r";
        case "info": return "&7&linfo&r";
    }
}