...
/**
 * Total over `Importable["type"]` via `satisfies`, so a new importable type fails
 * to compile until it declares how it is pruned or opts out with `null`.
 *
 * ITEM opts out: an item exists only where an action or menu references it, so
 * there is nothing to enumerate or delete.
 */
const PRUNE_TYPES = {
    FUNCTION: {
        type: "FUNCTION",
...
/** Scan order: cheap list reads before menu walks. */
