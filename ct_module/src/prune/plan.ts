...
export type PruneScanRequest = {
     ...
     * from a parse with no hard errors: anything a half-parse dropped would read
     * as undeclared house content.
     */
...
/**
 * What one save changed, decided from the lock alone (no house scan, no command
 * cooldown). Everything found is owned: the lock only records what a previous
 * import of this project created.
 *
 * One name gone and one arrived, for a type Housing can rename, is usually a
 * rename; a wrong guess falls back to remove-and-recreate.
 */
