...
export type HtslLine = {
    ...
    actionIndex: number;
    /** Inner action path, e.g. `4.ifActions.2`; top-level lines use `4`. */
    actionPath: string;
    /** Indent level (inner actions inside CONDITIONAL/RANDOM bodies). */
    depth: number;
...
export type ParsedFile = {
    ...
    /** Span table for the parsed actions (and their inner actions). Null on parse error. */
 ...
 */
function collectInnerActionPaths(action: Action, basePath: string): string[] {
    const out: string[] = [];
    function addChildren(actions: readonly Action[] | undefined, prop: string): void {
        ...
        for (let i = 0; i < actions.length; i++) {
            ...
            out.push(path);
            const inner = collectInnerActionPaths(actions[i], path);
...
function actionToLines(action: Action, actionIndex: number): HtslLine[] {
    ...
    const out: HtslLine[] = [];
    const raw = src.split("\n");
    const innerActionPaths = collectInnerActionPaths(action, basePath);
    let innerActionCursor = 0;
    for (let i = 0; i < raw.length; i++) {
        ...
        if (depth > 0 && !isStructuralLine(text) && innerActionCursor < innerActionPaths.length) {
            actionPath = innerActionPaths[innerActionCursor];
            innerActionCursor++;
        }
